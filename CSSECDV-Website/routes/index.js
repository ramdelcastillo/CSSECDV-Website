...
router.get("/profile/:username/signed-url", async (req, res) => {
  try {
    ...
    const token = jwt.sign({ username }, process.env.IMAGE_SECRET, { expiresIn: "10s" });
...
// router.post("/fdf1b191a3e96b81f5fa5761fa57379c", express.json(), loginUser);
...
router.get("/posts/:slug/signed-url", async (req, res) => {
  try {
    ...
    const token = jwt.sign({ slug }, process.env.IMAGE_SECRET, { expiresIn: "10s" });
